...
<h2 class="ta-c wra-h">查询支付虚拟币记录</h2>
   ...
   <div>
  		...
  		<%
  		 ...
  		 %>
         <a href="${pageContext.request.contextPath }/gameUserAction_findAll.action?currentPage=1">首页</a>
         <%
            if(currentPage!=1){
          %>
          <a href="${pageContext.request.contextPath }/gameUserAction_findAll.action?currentPage=<%=currentPage-1%>">上一页</a>
          <%
             }
           %>
         <%
          ...
          %>
         <a href="${pageContext.request.contextPath }/gameUserAction_findAll.action?currentPage=<%=currentPage+1%>">下一页</a>
         <%
             }
          %>
          <%
             if(currentPage!=pages.getPageTotal()){
          %>
          <a href="${pageContext.request.contextPath }/gameUserAction_findAll.action?currentPage=<%=pageTotal%>">尾页</a>
          <%
             }
          %>
      </div>
