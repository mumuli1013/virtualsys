...
<h2 class="ta-c wra-h">查询充值记录</h2>
   ...
   <a href="${pageContext.request.contextPath }/rechargeOrderAction_findAll.action">查询所有订单</a>
   <a href="${pageContext.request.contextPath }/rechargeOrderAction_findAllByStatus.action?status=1">查询所有成功订单</a>
   <table border="1">
       <tr>
          <th>充值时间</th>
          <th>充值账号</th>
   ...
   <div>
  		...
  		<%
  		 ...
  		 %>
         <a href="${pageContext.request.contextPath }/rechargeOrderAction_findByTime.action?currentPage=1&userId=<s:property value="#session.existUser.id"/>&startTime=<s:property value="#session.startTime"/>&endTime=<s:property value="#session.endTime"/>">首页</a>
         <%
            if(currentPage!=1){
          %>
          <a href="${pageContext.request.contextPath }/rechargeOrderAction_findByTime.action?currentPage=<%=currentPage-1%>&userId=<s:property value="#session.existUser.id"/>&startTime=<s:property value="#session.startTime"/>&endTime=<s:property value="#session.endTime"/>">上一页</a>
          <%
             }
           %>
         <%
          ...
          %>
         <a href="${pageContext.request.contextPath }/rechargeOrderAction_findByTime.action?currentPage=<%=currentPage+1%>&userId=<s:property value="#session.existUser.id"/>&startTime=<s:property value="#session.startTime"/>&endTime=<s:property value="#session.endTime"/>">下一页</a>
         <%
             }
          %>
          <%
             if(currentPage!=pages.getPageTotal()){
          %>
          <a href="${pageContext.request.contextPath }/rechargeOrderAction_findByTime.action?currentPage=<%=pageTotal%>&userId=<s:property value="#session.existUser.id"/>&startTime=<s:property value="#session.startTime"/>&endTime=<s:property value="#session.endTime"/>">尾页</a>
          <%
             }
          %>
      </div>
