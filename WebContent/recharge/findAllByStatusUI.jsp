...
<h2 class="ta-c wra-h">查询充值记录</h2>
   ...
   <div>
  		...
  		<%
  		 ...
  		 %>
         <a href="${pageContext.request.contextPath }/rechargeOrderAction_findAllByStatus.action?currentPage=1&status=<s:property value="#session.status"/>">首页</a>
         <%
            if(currentPage!=1){
          %>
          <a href="${pageContext.request.contextPath }/rechargeOrderAction_findAllByStatus.action?currentPage=<%=currentPage-1%>&status=<s:property value="#session.status"/>">上一页</a>
          <%
             }
           %>
         <%
          ...
          %>
         <a href="${pageContext.request.contextPath }/rechargeOrderAction_findAllByStatus.action?currentPage=<%=currentPage+1%>&status=<s:property value="#session.status"/>">下一页</a>
         <%
             }
          %>
          <%
             if(currentPage!=pages.getPageTotal()){
          %>
          <a href="${pageContext.request.contextPath }/rechargeOrderAction_findAllByStatus.action?currentPage=<%=pageTotal%>&status=<s:property value="#session.status"/>">尾页</a>
          <%
             }
          %>
      </div>
