...
<h2 class="ta-c wra-h">查询支付虚拟币记录</h2>
   ...
   <div>
  		...
  		<%
  		 ...
  		 %>
         <a href="${pageContext.request.contextPath }/payAction_findById.action?currentPage=1&userId=<s:property value="#session.existUser.id"/>">首页</a>
         <%
            if(currentPage!=1){
          %>
          <a href="${pageContext.request.contextPath }/payAction_findById.action?currentPage=<%=currentPage-1%>&userId=<s:property value="#session.existUser.id"/>">上一页</a>
          <%
             }
           %>
         <%
          ...
          %>
         <a href="${pageContext.request.contextPath }/payAction_findById.action?currentPage=<%=currentPage+1%>&userId=<s:property value="#session.existUser.id"/>">下一页</a>
         <%
             }
          %>
          <%
             if(currentPage!=pages.getPageTotal()){
          %>
          <a href="${pageContext.request.contextPath }/payAction_findById.action?currentPage=<%=pageTotal%>&userId=<s:property value="#session.existUser.id"/>">尾页</a>
          <%
             }
          %>
      </div>
