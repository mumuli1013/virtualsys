...
<h2 class="ta-c wra-h">查询支付虚拟币记录</h2>
   ...
   <div>
  		...
  		<%
  		 ...
  		 %>
         <a href="${pageContext.request.contextPath }/payAction_findAllByTime.action?currentPage=1&startTime=<s:property value="#session.startTime"/>&endTime=<s:property value="#session.endTime"/>">首页</a>
         <%
            if(currentPage!=1){
          %>
          <a href="${pageContext.request.contextPath }/payAction_findAllByTime.action?currentPage=<%=currentPage-1%>&startTime=<s:property value="#session.startTime"/>&endTime=<s:property value="#session.endTime"/>">上一页</a>
          <%
             }
           %>
         <%
          ...
          %>
         <a href="${pageContext.request.contextPath }/payAction_findAllByTime.action?currentPage=<%=currentPage+1%>&startTime=<s:property value="#session.startTime"/>&endTime=<s:property value="#session.endTime"/>">下一页</a>
         <%
             }
          %>
          <%
             if(currentPage!=pages.getPageTotal()){
          %>
          <a href="${pageContext.request.contextPath }/payAction_findAllByTime.action?currentPage=<%=pageTotal%>&startTime=<s:property value="#session.startTime"/>&endTime=<s:property value="#session.endTime"/>">尾页</a>
          <%
             }
          %>
      </div>
