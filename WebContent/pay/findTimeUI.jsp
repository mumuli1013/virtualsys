...
<h2 class="ta-c wra-h">查询支付虚拟币记录</h2>
   <s:form action="payAction_findByTime" method="get" namespace="/">
   <input type="hidden" value="<s:property value="#session.existUser.id"/>" name="userId"/>
      ...
      <a href="${pageContext.request.contextPath }/payAction_findByIs.action?userId=<s:property value="#session.existUser.id"/>&status=0">查询成功订单</a>
   <a href="${pageContext.request.contextPath }/payAction_findAll.action">查询所有订单</a>
   <a href="${pageContext.request.contextPath }/payAction_findAllByStatus.action?status=0">查询所有成功订单</a>
   ...
   <div>
  		...
  		<%
  		 ...
  		 %>
         <a href="${pageContext.request.contextPath }/payAction_findByTime.action?currentPage=1&userId=<s:property value="#session.existUser.id"/>&startTime=<s:property value="#session.startTime"/>&endTime=<s:property value="#session.endTime"/>">首页</a>
         <%
            if(currentPage!=1){
          %>
          <a href="${pageContext.request.contextPath }/payAction_findByTime.action?currentPage=<%=currentPage-1%>&userId=<s:property value="#session.existUser.id"/>&startTime=<s:property value="#session.startTime"/>&endTime=<s:property value="#session.endTime"/>">上一页</a>
          <%
             }
           %>
         <%
          ...
          %>
         <a href="${pageContext.request.contextPath }/payAction_findByTime.action?currentPage=<%=currentPage+1%>&userId=<s:property value="#session.existUser.id"/>&startTime=<s:property value="#session.startTime"/>&endTime=<s:property value="#session.endTime"/>">下一页</a>
         <%
             }
          %>
          <%
             if(currentPage!=pages.getPageTotal()){
          %>
          <a href="${pageContext.request.contextPath }/payAction_findByTime.action?currentPage=<%=pageTotal%>&userId=<s:property value="#session.existUser.id"/>&startTime=<s:property value="#session.startTime"/>&endTime=<s:property value="#session.endTime"/>">尾页</a>
          <%
             }
          %>
      </div>
