...
<h2 class="ta-c wra-h">查询支付虚拟币记录</h2>
   <s:form action="payAction_findGIdByTime" method="get" namespace="/">
   <input type="hidden" value="<s:property value="#session.existUser.gameId"/>" name="gameId"/>
       <div class="ta-c">
       <b>开始日期：</b>
      ...
      <a href="${pageContext.request.contextPath }/payAction_findGIdByStatus.action?gameId=<s:property value="#session.existUser.gameId"/>&status=0">查询成功订单</a>
   <table border="1">
       <tr>
          <th>支付时间</th>
          <th>支付账号</th>
   ...
   <div>
  		...
  		<%
  		 ...
  		 %>
         <a href="${pageContext.request.contextPath }/payAction_findByGameId.action?currentPage=1&gameId=<s:property value="#session.existUser.gameId"/>">首页</a>
         <%
            if(currentPage!=1){
          %>
          <a href="${pageContext.request.contextPath }/payAction_findByGameId.action?currentPage=<%=currentPage-1%>&gameId=<s:property value="#session.existUser.gameId"/>">上一页</a>
          <%
             }
           %>
         <%
          ...
          %>
         <a href="${pageContext.request.contextPath }/payAction_findByGameId.action?currentPage=<%=currentPage+1%>&gameId=<s:property value="#session.existUser.gameId"/>">下一页</a>
         <%
             }
          %>
          <%
             if(currentPage!=pages.getPageTotal()){
          %>
          <a href="${pageContext.request.contextPath }/payAction_findByGameId.action?currentPage=<%=pageTotal%>&gameId=<s:property value="#session.existUser.gameId"/>">尾页</a>
          <%
             }
          %>
      </div>
